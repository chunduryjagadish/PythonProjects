...
def bubbleSort(unsortedList,order):
    ...
    print("arraySize " + str(arraySize) + "\n Unsorted List " + str(unsortedList))
    numberOfLoops = 0
    sorted = False
    for i in range(0,arraySize-1):
        ...
        for j in range(0, arraySize-1-i):
            if (order == DECENDING):
                ...
                if (order == ASCENDING):
                    if (unsortedList[j] < unsortedList [j+1]):
                        ...
                        unsortedList[j] = temp
                        sorted = False
    print("Sorted List " + str(unsortedList) + "\nnumberOfLoops = " + str(numberOfLoops))
    return unsortedList

def bubbleSortAutoTest():
    printString = "Not Sorted: Order =  {} \n {}"
    ...
    order = DECENDING
    # order = ASCENDING
    sortedList = bubbleSort(unsortedList,order)
    if (order == ASCENDING):
        for i in range(numberOfElements-1):
            if sortedList[i] < sortedList[i+1]:
                print(printString.format(order, sortedList))
                sorted = False
                break
    else:
        for i in range(numberOfElements-1):
